...
export class FiltersService {
	...
	static calculatePotentialOverlaysCount(metadataKey: IFilter, metadata: FilterMetadata, overlays: Map<string, IOverlay>, favoriteOverlays: IOverlay[], removedOverlays: IOverlay[], removedOverlaysVisibility: boolean, filterState: IFiltersState): void {
		...
		if (metadata instanceof EnumFilterMetadata) {
			Array.from(metadata.enumsFields)
				...
				.forEach(([enumFiledKey, value]: [any, IEnumFiled]) => {
					...
					this.calculateOverlaysCount(metadataKey, metadata, overlays, favoriteOverlays, removedOverlays, removedOverlaysVisibility, filterState, (<EnumFilterMetadata>cloneMetadata));
				});
		} else {
			if ((<BooleanFilterMetadata>metadata).properties.true.value === false || (<BooleanFilterMetadata>metadata).properties.false.value === false) {
				...
				this.calculateOverlaysCount(metadataKey, metadata, overlays, favoriteOverlays, removedOverlays, removedOverlaysVisibility, filterState, cloneMetadata);
			}
		}
	}

	static calculateOverlaysCount(metadataKey: IFilter, metadata: FilterMetadata, overlays: Map<string, IOverlay>, favoriteOverlays: IOverlay[], removedOverlays: IOverlay[], removedOverlaysVisibility: boolean, filterState: IFiltersState, cloneMetadata: FilterMetadata): void {
		...
		const filteredOverlays: string[] = OverlaysService.buildFilteredOverlays(Array.from(overlays.values()), filterModels, favoriteOverlays, filterState.facets.showOnlyFavorites, removedOverlays, removedOverlaysVisibility );
