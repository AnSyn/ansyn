...
import { IIcon } from '../../../../../../../imagery/model/visualizers/visualizer-style';
...
export class FootprintPolylineVisualizer extends BaseFootprintsVisualizer {
	...
	private getFeatureType(feature: Feature): 'MultiLineString' | 'Point' | 'LineString' | 'MultiPolygon' {
		return feature && feature.getGeometry().getType();
	}
	...
	private getImage(feature, hover = false): IIcon | undefined {
		if (this.getFeatureType(feature) === 'LineString') {
			...
			return {
				src: this.getIconSvg(hover ? this.colorWithAlpha(color, 0.5) : color),
				anchor: [0.5, 0.5],
				scale: 0.15,
				rotation,
				rotateWithView: true
	...
	getIconSvg(mainColor: string, gradientColor: string = '#8cceff') {
