...
describe('VisualizersAppEffects', () => {
	...
	it('onStartMapShadow$ should listen to start shadow action', () => {
		spyOn(visualizersAppEffects, 'clearShadowMouse');
		spyOn(visualizersAppEffects, 'setShadowMouseProducer');
		spyOn(visualizersAppEffects, 'addShadowMouseConsumer');
		actions = hot('--a--', { a: new StartMouseShadow() });

		const expectedResults = cold('--b--', { b: undefined });

		expect(visualizersAppEffects.onStartMapShadow$).toBeObservable(expectedResults);
		expect(visualizersAppEffects.clearShadowMouse).toHaveBeenCalledTimes(3);
		expect(visualizersAppEffects.setShadowMouseProducer).toHaveBeenCalledTimes(1);
		expect(visualizersAppEffects.addShadowMouseConsumer).toHaveBeenCalledTimes(2);
	});

	fit('onEndMapShadow$ should stop listen map shadow action', () => {
		spyOn(visualizersAppEffects, 'clearShadowMouse');
		actions = hot('--a--', { a: new StopMouseShadow() });

		const expectedResults = cold('--b--', { b: undefined });

		expect(visualizersAppEffects.onEndMapShadow$).toBeObservable(expectedResults);
		expect(visualizersAppEffects.clearShadowMouse).toHaveBeenCalledTimes(3);
	});
});
