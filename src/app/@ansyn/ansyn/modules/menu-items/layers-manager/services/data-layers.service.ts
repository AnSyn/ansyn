...
export class DataLayersService implements OnInit, OnDestroy {
	...
	addLayer(layer: ILayer): Observable<any> {
		return this.storageService.create('layers', { preview: layer }).pipe(
			tap(() => {
				this.store.dispatch(AddLayerOnBackendSuccessAction({layerId: layer.id}));
			}),
			catchError((err) => {
				this.store.dispatch(AddLayerOnBackendFailedAction({layer, error: err}));
	...
	updateLayer(layer: ILayer): Observable<any> {
		return this.storageService.update('layers', { preview: layer, data: null }).pipe(
			tap(() => {
				this.store.dispatch(UpdateLayerOnBackendSuccessAction({layerId: layer.id}));
			}),
			catchError((err) => {
				this.store.dispatch(UpdateLayerOnBackendFailedAction({layer, error: err}));
	...
	removeLayer(layerId: string): Observable<any> {
		return this.storageService.delete('layers', layerId)
			.pipe(
				tap(() => {
					this.store.dispatch(RemoveLayerOnBackendSuccessAction({layerId}));
				}),
				catchError((err) => {
					this.store.dispatch(RemoveLayerOnBackendFailedAction({layerId, error: err}));
	...
	removeCaseLayers(caseId: string): Observable<any> {
		this.store.dispatch(RemoveCaseLayersFromBackendAction({caseId}));
