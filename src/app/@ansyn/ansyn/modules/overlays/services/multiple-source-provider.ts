...
export class MultipleOverlaysSourceProvider {
	...
	public fetch(fetchParams: IFetchParams): Observable<IOverlaysFetchData> {
		const mergedSortedOverlays: Observable<IOverlaysFetchData> = forkJoin(this.sourceConfigs
			.filter(s => !Boolean(fetchParams.dataInputFilters.length) ? true : fetchParams.dataInputFilters.some((dataInputFilter: IDataInputFilterValue) => dataInputFilter.name === s.provider.sourceType))
