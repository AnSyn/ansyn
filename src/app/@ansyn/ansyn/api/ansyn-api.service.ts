...
export class AnsynApi {
	...
	events = {
		 ...
		 * @property overlays - array of the loaded overlays or false if there was error.
		 */
		overlaysLoadedSuccess: new EventEmitter<IOverlay[] | false>(),
		/**
		 * fire when an overlay was loaded to the map
		 * @property the loaded overlay or false if there was error on open the overlay.
	...
	/**
	 * @return an observable that emit the current mouse position on the map.
	 */
	...
	/**
	 * @return An observable that emit when the map layout is change
	 */
	onSetLayoutSuccess$: Observable<any> = this.actions$.pipe(
		ofType(MapActionTypes.SET_LAYOUT_SUCCESS)
	);
	/**
	 * @return An observable that emit when the center of the map is change
	 ...
	 * @param geoPoint The point where we will draw the visualizer.
	 */
	 ...
	 * @param overlay the overlay will be display
	 * @param mapId the map number or id. default the active map id.
	 */
	displayOverLay(overlay: IOverlay, mapId?: mapIdOrNumber): void {
		...
		this.store.dispatch(new DisplayOverlayAction({
			overlay,
	 ...
	 * @param featureCollection the features to draw on the map.
	 */
	 ...
	 * @param overlays array of IOverlay to be load into the timeline
	 */
	 ...
	 * @param layout the new layout key.
	 ...
	 * @param position the position in the old coordinate system
	 * @param convertFrom the current coordinate system of the position
	 * @param convertTo the new coordinate system to the position
	 * @return the position in the new coordinate system.
	 */
	 ...
	 * @param geoPoint the new center for the map.
	 * @param mapId the map id or number, default the active map id.
	 */
	 ...
	 * @param rect a rectangle polygon
	 * @param mapId the id of the map to be set. default the active map id.
	 */
	 ...
	 * @param radian The rotation of the map in radians.
	 * @param mapId the id of the map. default to the active map.
	 */
	 ...
	 * @param mapId the id of the map. default to the active map.
	 * @return the map rotation in radians.
	 */
	getRotation(mapId?: mapIdOrNumber): number {
	 ...
	 * @param center GeoJSON point to be set the map to.
	 * @param radiusInMeters set the map scale to radiusInMeters / 10
	 * @param mapId the id of the map to be set. default the active map id.
	 * @param search does make new search in the new position or not.
	 */
	setMapPositionByRadius(center: Point, radiusInMeters: number, mapId: mapIdOrNumber, search: boolean = false): void {
		if (!Boolean(center)) {
			console.error('can\'t set position to undefined point');
			return null;
		}
		this.store.dispatch(new SetMapPositionByRadiusAction({
			id: this.getMapIdFromMapNumber(mapId),
			center,
			radiusInMeters
		}));
		if (search) {
	 ...
	 * @param criteria of the search. you can pass only one criterion.
	 * @param [radiusInMetersBuffer] if the region criterion is provide, bump the region by the radius.
	 */
	...
	/**
	 * @return An Observable that emits when the search criteria was change.
	 */
	...
	/**
	 * @return the overlay that display on the map, or undefined if no overlay was display
	 ...
	 * @param collapse true for minimize the footer, or false to maximize.
	 */
	 ...
	 * @param collapse true for minimize the menu, or false to maximize.
	 */
	 ...
	 * @param collapse true to set the app to minimalist view, false to set the app to normal view.
	 */
	 ...
	 * @param layerData the features to be add into the layer.
	 * @param isEditable set if this layer was editable or not. default true
	 * @return the layer id. for future deletion of the layer.
	 */
	 ...
	 * @param layerId the id of the layer who return from
	 */
	 ...
	 * @param layerId the id of the layer who return from
	 * @param show true to show the layer, false to hide.
	 */
