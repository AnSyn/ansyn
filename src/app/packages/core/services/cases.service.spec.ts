...
describe('CasesService', () => {
  ...
  it('createCase should get case and send the case as body in ajax("post"), result should unshift to cases and call caseAdded.emit', () => {
    ...
    expect(http.post).toHaveBeenCalledWith(`${casesService.base_url}/cases`, JSON.stringify(selected_case), new RequestOptions({ headers: new Headers({ 'Content-Type': 'application/json' })}));
  ...
  it('updateCase should get case and send the case as body in ajax("put"), result should update the selected case on cases', () => {
    ...
    expect(http.put).toHaveBeenCalledWith(`${casesService.base_url}/cases`, JSON.stringify(selected_case), new RequestOptions({ headers: new Headers({ 'Content-Type': 'application/json' })}));
    expect(casesService.cases[0]).toEqual(selected_case);
  });

  it('removeCase should get case_id and send case_id as body in ajax("delete"), result should update the selected case on cases', () => {
    ...
    expect(http.delete).toHaveBeenCalledWith(`${casesService.base_url}/cases/${case_id_to_remove}`, new RequestOptions({ headers: new Headers({ 'Content-Type': 'application/json' })}));
